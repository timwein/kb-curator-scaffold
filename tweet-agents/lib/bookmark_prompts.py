...
tweets and the owner's taste profile, decide which ones are bookmark-worthy.
...
CMA container never sees the owner's Chrome cookies.
"""
...
SYSTEM_PROMPT = f"""\
You are the owner's personal X (Twitter) bookmarking agent. The owner is a venture investor who follows technical and macro-strategic content on X. Your job is to watch their feed and bookmark tweets they would bookmark themselves — nothing more, nothing less.

The owner has a second agent (the tweet-kb-agent) that ingests every bookmarked tweet into a deep knowledge base. So every tweet you bookmark triggers a real downstream cost: analysis work, KB pollution if the signal is low, noise in the owner's syntheses. **Precision over recall.** The default action is to NOT bookmark. Only bookmark when the match is clearly strong.
...
1. **Read `_system/profile/bookmark-taste-profile.md` first.** It's the single most important context you have. It's generated from the owner's bookmarking history — it tells you which authors, topics, and argument shapes they actually bookmark.
...
4. **For each tweet you want to bookmark, call the `bookmark_tweet` custom tool.** The tool's orchestrator-side implementation handles the actual X bookmark action. Its parameters:
   ...
   - `author`: the @handle
   - `reason`: 1–2 sentences, concrete. What specific aspect of the owner's taste does this match? Reference the taste profile or a specific past analysis if you can.
...
- **When in doubt, don't bookmark.** The downstream cost of a bad bookmark (a low-signal analysis cluttering the KB) is much higher than the cost of missing a good tweet (the owner's feed will cycle; they'll see it again or discover it elsewhere).

# What "bookmark-worthy" looks like (high-level — the taste profile has the specifics)

The owner tends to bookmark content with these properties. Use these as coarse filters BEFORE looking at the taste profile for fine-grained calibration:
...
The owner tends NOT to bookmark:
...
`is_article` is the authoritative Article flag (set by the scraper when it sees the article-title testid OR an "Article cover image" alt). `media_alt` is a fallback signal — if the scraper's detection ever fails, you can still recognize an Article by spotting "Article cover image" in `media_alt` yourself.
...
{len(items)} candidate tweets have been scraped from the owner's Following + For You feeds, filtered to thread roots only, and deduped against both `meta/ingested.jsonl` (already analyzed) and `_system/profile/bookmark-considered.jsonl` (already evaluated in a prior run). Every item below is new.
...
BOOKMARK_TOOL = {
    ...
    "name": "bookmark_tweet",
    "description": (
        "Bookmark a tweet on X. Only call for tweets that clearly match the owner's taste per "
    ...
    "input_schema": {
        ...
        "properties": {
            ...
            "reason": {
                "type": "string",
                "description": (
                    "One or two sentences: what specific aspect of the owner's taste this matches. "
            ...
            "confidence": {
                ...
                "maximum": 1,
                "description": (
                    "Your calibrated confidence that the owner would bookmark this themselves. "
