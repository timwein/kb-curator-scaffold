...
decide what to bookmark. It aggregates the owner's bookmarking history (tweet
...
One-shot. Re-run whenever the owner wants to refresh the profile (e.g., after
...
def render_profile(
    ...
    lines: list[str] = []
    lines.append("# Bookmark Taste Profile")
    lines.append("")
    lines.append(
        "*Generated from the owner's bookmarking history. "
    ...
    lines.append("")
    lines.append(
        "This document is the single source of truth for what the owner tends to bookmark. "
    ...
    # --- Patterns prose ------------------------------------------------------
    lines.append("## What the owner Bookmarks (in prose)")
    lines.append("")
    lines.append(
        "The owner is a venture investor. Their bookmarks skew heavily toward: "
        ...
        "their own writing — so anything they bookmark should be something they can "
        "*quote*, not just read."
    )
    lines.append("")
    ...
    lines.append("- Tweets from authors who aren't in the owner's favored-author list, without unusually strong substance to compensate")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "Themes extracted from 200 past Claude.ai conversations the owner had about "
...
def main() -> None:
    ...
    try:
        ...
        # Vercel's committer-to-GitHub-user check passes; keep the distinct
        # user.name so bookmarker-setup commits are still identifiable.
